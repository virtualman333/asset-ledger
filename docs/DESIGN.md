...
上表是**选型范围**，不是已接清单。已经接上的只有：腾讯（A 股 / 港股 / 美股）、
OKX（加密）、Yahoo（美股备用）、Frankfurter + `open.er-api`（汇率）——
东财 push2 / Stooq / 天天基金 / CoinGecko / 股息日历源都还没写，
所以「基金/ETF」与「股息日历」两行现在拿不到自动价格（基金只能手填）。

统一的数据源接线在 `market/services.py` 的 `SOURCE_ORDER`（名字 + 取价函数），
`fetch_quote(asset, skip=...)` 按顺序试、任何一家挂了自动落到下一家，都失败就返回 `None`，
由上层用上一次的旧价兜底（`stale`）；批量走 `fetch_quotes()`。端上只向后端要价，
不直连第三方（避免跨域与 Key 泄露）。

**没做失败退避**（一家连续失败也只会照常重试），要按这一点估容量，别按退避算。

**落库只有一个入口**：`market/services.py::refresh_quote_map()`（单只的情形走 `refresh_quote()`，
两者共用同一份「查缓存 → 取价 → 落库」）—— 它同时负责「缓存期内直接复用」的判据。
定时任务（`market/scheduler.py`，`QUOTE_REFRESH_MINUTES` 分钟一轮）、
`manage.py refresh_quotes`、`GET /market/quotes/` 三处都调它，谁也不许自己 `create` 一条快照。

**一轮的请求条数才是限流的实际口径。** 腾讯接口本身就吃逗号分隔的多代码，所以
A 股 / 港股 / 美股按市场**合并成一条请求**（每 60 个代码一条，`--batch-size 1` 可退回逐只排查用）；
批量里没回来的再逐只走**剩下的**源（问过腾讯的不再问第二遍，否则每只失败的票都变成两条请求）。
代码规范化与批量解析在 `market/tencent.py`（纯函数、不 import django，所以「一批只发一条请求」
能靠注入假传输离线断言）。`USB`（美国合众银行）这类**本身带 `us` 两个字母**的真实代码，
与「`us` 前缀 + ticker」在字符串上无法区分，判据是「前缀后面必须全大写且 ≥2 位」，
两头都不像的返回 `None` 而不是猜：接口区分大小写（`usaapl` 回 `v_pv_none_match`），
猜错的结果是静默无价，比少一个价更坏。

这条约定不是洁癖：抓价这件事一旦有两个入口，缓存判据、失败兜底、`stale` 标记就会各长一套。
曾经的真实后果比这更严重 —— 除了定时任务压根不存在，那唯一一个入口还只被手工调试用过
（端上从不调那个接口），另有一条一次性的 `seed_demo` 路径。于是行情只在有人手动点一下的
...
| **拿不到报价的持仓** | **按成本计入**总资产与 XIRR 终值，并在 `valuation_note` 里点名是哪几个标的。按 0 计等于宣布这笔钱蒸发了 —— 年化会给出巨额负收益且不报错 |
| 视图 | 总资产走势、各市场占比、个股盈亏排行、月度股息柱状图 + 同比 |

估值口径（含「哪些持仓没报价」）只有一处定义：`apps/analytics/valuation.py`（纯函数，有单测）。
`build_summary` 的汇总字段与 XIRR 终值共用它算出来的同一份结果 ——
终值若是只按「有报价的市值」算，一旦持仓拿不到价，现金流里就少了这一笔，
年化会把「本金还在」读成「本金没了」，然后把原因写成"现金流不足"，把排查方向指错。
