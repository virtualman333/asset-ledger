...
这一轮又补了第四面：**动词**。原先只问「这条路径在不在」，于是
`ApiClient.post('/analytics/summary/')` 是合法的 —— 路径是真的、方法不是，真机上收到的是
405，界面同样只显示一句「加载失败」，而这台机器上没有鸿蒙工具链、`.ets` 编译不了。
现在每一处调用的动词也要落在真值上。真值来自 `route_inventory.route_methods()`：
先把「哪个视图接这个路径」解析出来（`X.as_view()` / 模块级别名 / 裸函数三种写法都跟得上），
再按视图类的基类与 mixin 推允许的动词；router 注册的资源还分**集合 URL** 与**明细 URL**
...
脚本能抓的只有「表本身错/腐烂」（基类表少写一个动词、漏了一种基类）。而这类错在单测里
...
列的集合是一份**划分**：每一列要么被这一行读走，要么被**声明**忽略，没有第三类。
导出那边加一列而导入这边还没安排，就会当场回 400 并点名是哪一列
（`这些列还不在导入的处理范围内：新列（brand_new）…`）—— 而不是照收不误、
把那一列的值悄悄丢掉让用户拿到一屏「成功」。这条以前只是注释里的君子协定：
`CONSUMED_KEYS` 是一份手抄的 12 项清单，`IGNORED_KEYS` 手抄 2 项，两者**全仓都没有消费方**。
现在消费集从 `CSV_COLUMNS` 现算，判断在 `resolve_headers()` 里真的被调用，
`test_import_rules.py::ColumnCoverageTest` 再按行为核对一遍（换掉某一格的值，
`build_row()` 的输出会不会变）。
...
第一列那个动词也不是装饰：同一份检查现在会拿 `route_inventory.route_methods()` 推出来的
真值核一遍「文档写了、服务端接不接」。给一条只读路由写 `POST`，照着这份文档写完一整个页面
才在真机上撞 405 —— 与「路径写错撞 404」同族，只是更晚才发现。反向（服务端接了、文档没写）
**不查**：这份清单列的是主要用法，而 `/api/v1/health/` 那种裸函数视图真值是五个动词全接。
...
- 收益口径的**展示面**（统计页的 `总盈亏` / `近一年股息` / `股息率` / `月度被动收入`，持仓行上的
  `近一年股息` / `股息率` 与账户名）由 `test_client_model_contract.py` 钉住：「后端会发、客户端
  没声明」与「声明了却没渲染」两向都红。**这一面此前是空的** —— `dividend_yield` /
  `monthly_passive_income` 曾经就写在「已知约束」这一条里，一条写下来再没人看的待办。
  断言用的真值两边都是现算的（后端从 `analytics/services.py` 的字典字面量、客户端从
  `Models.ets`），不是清单
...
- 承接上一条：本机没有鸿蒙工具链，`.ets` 的正确性**只**能靠读源码的契约检查兜底。目前
  钉住的是五面 —— 「客户端页面清单 / import 落点 / 路由登记」（`test_client_pages_contract.py`，
  ...
  「客户端打的每个接口路径都真实存在」，以及**每一处调用用的动词都被那条路由接**
  （`test_client_route_contract.py`，路径与动词都是宽窄两遍逐文件对账；动词那一面的真值
  与 Django 本体对过账），以及「`Models.ets` 的 `interface` 声明 ⇄ 后端真实返回的字段」
  再加「这几个数真的渲染出来了」（`test_client_model_contract.py`：两向对账 + 可空性 +
  显示面棘轮 —— 后者防的是「声明了但页面一行没读」，`total_pnl` 当初就是这个状态）
  ——**这五面没覆盖到的 `.ets` 改动，自测一律看不见**。
